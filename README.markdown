ESA
=================
[![Build Status](https://secure.travis-ci.org/mbulat/esa.png?branch=master)](http://travis-ci.org/mbulat/esa)

The ESA plugin is a Ruby on Rails Engine which provides a double entry accounting system for your application.

### NOTE: This version of ESA is compatible with RAILS 3.1

For the rails 2 version, you can go here:

[https://github.com/mbulat/esa/tree/rails2](https://github.com/mbulat/esa/tree/rails2)
...
- Add the gem to your Gemfile `gem "esa"`

- generate migration files `rails g esa`
...
The esa plugin provides a complete double entry accounting system for use in any Ruby on Rails application. The plugin follows general [Double Entry Bookkeeping](http://en.wikipedia.org/wiki/Double-entry_bookkeeping_system) practices. All calculations are done using [BigDecimal](http://www.ensta.fr/~diam/ruby/online/ruby-doc-stdlib/libdoc/bigdecimal/rdoc/classes/BigDecimal.html) in order to prevent floating point rounding errors. The plugin requires a decimal type on your database as well.
...
Your Book of Accounts needs to be created prior to recording any transactions. The simplest method is to have a number of create methods in your db/seeds.rb file like so:
    ...
    ESA::Asset.create(:name => "Accounts Receivable")
    ESA::Asset.create(:name => "Cash")
    ESA::Revenue.create(:name => "Sales Revenue")
    ESA::Liability.create(:name => "Unearned Revenue")
    ESA::Liability.create(:name => "Sales Tax Payable")
    etc...
...
See the {ESA::Account}, {ESA::Transaction}, and {ESA::Amount} classes for more information.
...
Next we'll build the transaction we want to record. ESA uses ActiveRecord conventions build the transaction and its associated amounts. 
...
Transactions must specify a description, as well as at least one credit and debit amount. `Amount`s must specify an amount value as well as an account, either by providing a `ESA::Account` to `account` or by passing in an `account_name` string.
...
Although ESA does not provide a mechanism for generating invoices or orders, it is possible to associate any such commercial document with a given transaction.

Suppose we pull up our latest invoice in order to generate a transaction for esa (we'll assume you already have an Invoice model):
...
To make the example clearer, imagine instead that the owner decides to invest his money into the business in exchange for some type of equity security. In this case we might have the following accounts:
    ...
    >> ESA::Equity.create(:name => "Common Stock")
    >> ESA::Asset.create(:name => "Cash")
...
The Engine provides controllers and views for viewing Accounts and Transactions via the {ESA::AccountsController} and {ESA::TransactionsController} classes. The controllers will render HTML, XML and JSON, and are compatible with [ActiveResource](http://api.rubyonrails.org/classes/ActiveResource/Base.html)
...
Routing is supplied via an engine mount point. ESA can be mounted on a subpath in your existing Rails 3 app by adding the following to your routes.rb:
...
Sample stylesheets can also be applied by adding the following to your application layout:
    ...
    <%= stylesheet_link_tag    "esa/application" %>
...
* Join the [mailing list](https://groups.google.com/d/forum/esa-gem) (Google Group)
